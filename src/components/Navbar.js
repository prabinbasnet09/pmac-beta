...
export default function NavbarStudent({ user, signOut }) {
  ...
  const handleSignOut = () => {
    ...
    Cookies.remove(
      `CognitoIdentityServiceProvider.2k4676qov74l0oovdtcfvpfff0.${user.username}.refreshToken`
    );
    Cookies.remove(
      `CognitoIdentityServiceProvider.2k4676qov74l0oovdtcfvpfff0.${user.username}.accessToken`
    );
    Cookies.remove(
      `CognitoIdentityServiceProvider.2k4676qov74l0oovdtcfvpfff0.LastAuthUser`
    );
    Cookies.remove(
      `CognitoIdentityServiceProvider.2k4676qov74l0oovdtcfvpfff0.${user.username}.idToken`
    );
                        ...
                        <Menu as="div" className="relative">
                          ...
                          <Menu.Items className="absolute right-0 z-5 mt-6 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-red ring-opacity-1 focus:outline-none">
                            {userNavigation.map((item) => (
                              <Menu.Item
                                key={item.name}
                                onClick={(e) => handleUserNavClick(e)}
                              >
                                {({ active }) => (
                                  <a
                                    href={item.href}
              ...
              <Disclosure.Panel className="md:hidden">
                <div className="mt-4 border-t  pt-4 pb-3">
                  <div className="flex items-center px-5">
                    ...
                    <div className="ml-3">
                      <div className="text-base font-medium leading-none text-gold">
                        {loggedUser.name}
                      </div>
